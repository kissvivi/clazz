  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        label="管理"
        align="center"
        v-if="hasPermission('student:detail') || hasPermission('student:update') || hasPermission('student:delete')"
      >
      ...
      <el-form-item label="学号" prop="code" required>
        ...
        <el-form-item label="密码" prop="password" required>
          <el-input
            ...
            prefix-icon="el-icon-edit"
            auto-complete="off"
            v-model="tempStudent.password"
          ></el-input>
        </el-form-item>

      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click.native.prevent="dialogStudentFormVisible = false">取消</el-button>
        <el-button
...
export default {
  ...
  data() {
    ...
    return {
      ...
      createRules: {
        name: [{ required: true, trigger: 'blur', validator: validateStudentName }],
        code: [{ required: true, trigger: 'blur', validator: validateStudentCode }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    addStudent() {
      this.$refs.tempStudent.validate(valid => {
        if (
          ...
          addStudent(this.tempStudent).then(() => {
    ...
    isStudentNameUnique(id, code) {
      for (let i = 0; i < this.studentList.length; i++) {
        if (this.studentList[i].id !== id && this.studentList[i].code === code) {
    ...
    cancelAll(index) {
      const handleList = this.permissionList[index].handleList
      for (let i = 0; i < handleList.length; i++) {
        const idIndex = this.tempStudent.permissionIdList.indexOf(handleList[i].id)
        if (idIndex > -1) {
